...
def _parse_date(value: object) -> str:
    """Parse ISO date, adjusting UTC midnight-offset to KST (UTC+9)."""
    s = _safe_str(value)
    if not s or "T" not in s:
        return s
    date_part, time_part = s.split("T", 1)
    if time_part.startswith("15:00:00") and time_part.endswith("Z"):
...
def format_thevc_company_markdown(info: TheVCCompanyInfo) -> str:
    ...
    if info.funding_rounds or info.last_round:
