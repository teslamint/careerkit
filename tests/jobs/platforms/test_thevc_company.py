...
class TestSafeInt:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("4", 4),
            ("abc", 0),
            ({"requirements": ["PLAN:PRO"]}, 0),
            (None, 0),
            ("42.5", 0),
            ("1e999", 0),
        ],
    )
    def test_safe_int_coerces_or_defaults_to_zero(self, raw, expected):
...
class TestThevcCompanyHttp:
    ...
    def test_members_non_list_scalar_yields_no_ceo(self):
        response = {**SAMPLE_API_RESPONSE, "members": 123}
        client = _FakeHttpClient(response)
        info = thevc_company_http("test", http=client)

        assert info.ceo_name == ""
        assert info.ceo_is_founder is False

    def test_investor_count_overflow_yields_zero(self):
        response = {**SAMPLE_API_RESPONSE, "investorCount": {"total": float("inf")}}
        client = _FakeHttpClient(response)
        info = thevc_company_http("test", http=client)
    ...
    def test_date_kst_day_boundary(self, utc_timestamp: str, expected_kst_date: str):
        response = {**SAMPLE_API_RESPONSE, "foundedOn": utc_timestamp}
        client = _FakeHttpClient(response)
        info = thevc_company_http("test", http=client)

        assert info.founded_on == expected_kst_date
...
class TestFormatThevcCompanyMarkdown:
    ...
    def test_funding_section_with_aggregates_only(self):
        """Investment section appears even without rounds when aggregates exist."""
        info = TheVCCompanyInfo(
            name="Test",
            total_funding_count=3,
            investor_count_total=5,
            slug="test",
        )
        md = format_thevc_company_markdown(info)

        assert "## 투자 정보" in md
        assert "| 총 라운드 수 | 3회 |" in md
        assert "| 누적 투자자 수 | 5곳 |" in md

    def test_last_funded_on_rendered(self):
        info = TheVCCompanyInfo(
            name="Test",
            last_funded_on="2024-03-15",
            slug="test",
        )
        md = format_thevc_company_markdown(info)

        assert "## 투자 정보" in md
        assert "| 최근 투자일 | 2024-03-15 |" in md
