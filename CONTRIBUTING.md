...
- 가드 브랜치 착륙 푸시처럼 새 한글 문서 줄이 측정을 넘으면 그 한 번만
  `GUARD_LAYERS=1,3`을 쓴다 — 그 줄들이 다음 병합 베이스의 어휘로 들어가면
  표기는 사라진다. CI에는 워크스페이스가 없으므로 착륙 PR의 저장소 변수는
  `GUARD_LAYERS=1`로 설정한다(1,3이면 레이어 3이 exit 2로 거부한다). 변수는
  착륙 후 삭제한다.
- `post-merge`, `post-rewrite`, `post-checkout` 훅이 레이어 2의 어휘 캐시를
  차단 경로 밖에서 갱신한다.
- 처음 올리는 커밋(리모트에 아직 이 브랜치의 커밋이 없을 때)은 `--exclude-remote`로
  해당 리모트의 레퍼가 이미 실은 커밋을 제외하고 정확히 새 커밋만 스캔한다.
- 가드의 정의 파일 자체(`src/careerkit/publish_guard.py`와 두 계약 테스트)는
  규칙 패턴을 소스 텍스트로 담고 있어서, 내용 스캔에서는 정확 경로 일치로만
  제외된다. 다른 이름으로 복사한 파일은 여전히 스캔되고, 커밋 메시지는
  어디에서도 제외되지 않는다.
